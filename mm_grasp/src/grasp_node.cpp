...
public:
  ...
  : node_(node), logger_(node->get_logger())
  {
    ...
    // 2026-08-13 一度再下压 5mm, 当天实跑后撤回: 卸货整段更贴地后, 放地释放的竖直退开
    //   被判起始态碰撞 (笛卡尔覆盖 0%), 整条任务中止. 详见 place.yaml 同名项注释.
    // 2026-08-14 用户要求再次下压 5mm 并"写死": 与 place.yaml 逐字同步改到这组值 ——
    //   ⚠️ 只改这里不改 yaml 是**无效**的 (getOrDeclare: yaml 有值就不用默认), 反之亦然.
    //   若又出现放地退开覆盖 0%, 两处一起回 -0.010/-0.010/-0.008/-0.012.
