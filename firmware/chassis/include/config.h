...
#define PID_KP   350
// ⚠️ KI 从 10 改 1000 不是"调大 100 倍": 同时把积分项改成了 error*dt (原来是裸 error 累加),
// dt=0.01 恰好抵掉这 100 倍, **等效积分增益与原来完全相同**, 响应不变。
// 改的意义在于 ki 从此与采样率解耦(原实现改 CONTROL_RATE_HZ 会悄悄改变积分强度),
// 且抗饱和上限 out_max/ki 变成有物理意义的量(255/1000=0.255 m/s·s, 与误差同量纲)。
#define PID_KI   1000
#define PID_KD   0.0
#define PID_OUT_LIMIT 255
// 积分分离阈值 (m/s): 误差超此值不积分, 只靠 KP 出力。
// 治"某轮不跟指令走、松手再按就好"(松手触发 failsafe 清积分, 见 main.cpp control_task)。
// 底盘限速 0.36m/s, 取 0.15 ≈ 半程: 起步/大阶跃不积分(那时该 KP 出力), 接近目标才积分消静差。
#define PID_INTEGRAL_SEP_BAND 0.15f
// 测速 EMA 低通系数: 100Hz+380PPR 下低速每tick仅~3脉冲, 不滤波会量化抖/换向抖
#define SPEED_FILTER_ALPHA 0.3f
// 单轮测速合理性上限 (m/s): 超过即判为编码器爆量, 丢弃该帧沿用上一值 (见 main.cpp 控制环)
// 底盘实际限速 0.36m/s, 单轮极限(含自转分量)约 0.5m/s; 取 2.0 = 4 倍余量,
// 只拦物理不可能值, 不干扰正常调速与打滑瞬态。2026-08-08 实测爆量达 143m/s, 远超此阈。
#define WHEEL_SPEED_SANE_MAX 2.0f
